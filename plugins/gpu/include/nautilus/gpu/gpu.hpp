#pragma once

#include "nautilus/function.hpp"
#include "nautilus/nautilus_function.hpp"
#include "nautilus/val.hpp"
#include "nautilus/val_ptr.hpp"
#include <cstddef>
#include <cstdint>

namespace nautilus::gpu {

// ============================================================================
// Kernel function type
// ============================================================================

/// A NautilusFunction subclass that explicitly marks the wrapped callable as a
/// GPU device kernel.  Using this type instead of plain NautilusFunction makes
/// the intent visible at the API level and allows the GPU backends to identify
/// kernels through the "kernel" attribute rather than heuristic intrinsic scanning.
template <class F>
class NautilusKernelFunction : public NautilusFunction<F> {
public:
	NautilusKernelFunction(std::string name, F f) : NautilusFunction<F>(std::move(name), std::move(f)) {
		this->definition_.setAttribute("kernel", "true");
	}

	NautilusKernelFunction(const NautilusKernelFunction&) = delete;
	NautilusKernelFunction& operator=(const NautilusKernelFunction&) = delete;
	NautilusKernelFunction(NautilusKernelFunction&&) = delete;
	NautilusKernelFunction& operator=(NautilusKernelFunction&&) = delete;
};

template <class F>
NautilusKernelFunction(std::string, F) -> NautilusKernelFunction<std::decay_t<F>>;

// ============================================================================
// Thread indexing intrinsics
// ============================================================================

/// Returns the thread index within the block for dimension x/y/z.
/// CPU fallback: returns 0 (single thread).
val<uint32_t> threadIdx_x();
val<uint32_t> threadIdx_y();
val<uint32_t> threadIdx_z();

/// Returns the block index within the grid for dimension x/y/z.
/// CPU fallback: returns 0 (single block).
val<uint32_t> blockIdx_x();
val<uint32_t> blockIdx_y();
val<uint32_t> blockIdx_z();

/// Returns the block dimension (threads per block) for dimension x/y/z.
/// CPU fallback: returns 1.
val<uint32_t> blockDim_x();
val<uint32_t> blockDim_y();
val<uint32_t> blockDim_z();

/// Returns the grid dimension (number of blocks) for dimension x/y/z.
/// CPU fallback: returns 1.
val<uint32_t> gridDim_x();
val<uint32_t> gridDim_y();
val<uint32_t> gridDim_z();

/// Block-level barrier synchronization.
/// CPU fallback: no-op.
void syncThreads();

// ============================================================================
// Kernel launch
// ============================================================================

/// Grid dimensions for kernel launch (number of thread blocks).
struct GridDim {
	val<uint32_t> x;
	val<uint32_t> y;
	val<uint32_t> z;

	/// 1D grid.
	GridDim(val<uint32_t> x) : x(x), y((uint32_t) 1), z((uint32_t) 1) {
	}

	/// 3D grid.
	GridDim(val<uint32_t> x, val<uint32_t> y, val<uint32_t> z) : x(x), y(y), z(z) {
	}
};

/// Block dimensions for kernel launch (threads per block).
struct BlockDim {
	val<uint32_t> x;
	val<uint32_t> y;
	val<uint32_t> z;

	/// 1D block.
	BlockDim(val<uint32_t> x) : x(x), y((uint32_t) 1), z((uint32_t) 1) {
	}

	/// 3D block.
	BlockDim(val<uint32_t> x, val<uint32_t> y, val<uint32_t> z) : x(x), y(y), z(z) {
	}
};

// Internal: traced launch config intrinsics.
void setGrid(val<uint32_t> x, val<uint32_t> y, val<uint32_t> z);
void setBlock(val<uint32_t> x, val<uint32_t> y, val<uint32_t> z);

/// Launch a NautilusKernelFunction as a GPU kernel with explicit grid/block dimensions.
///
/// The grid and block dimensions are traced values, so they can be computed
/// dynamically from data (e.g. `GridDim{(n + 255) / 256}`).
///
/// During tracing, this emits:
///   1. setGrid(grid.x, grid.y, grid.z)   → backend stores as launch config
///   2. setBlock(block.x, block.y, block.z) → backend stores as launch config
///   3. kernel(args...)                     → backend emits kernel launch with stored config
///
/// CUDA backend generates: `kernel<<<dim3(gx,gy,gz), dim3(bx,by,bz)>>>(args)`
/// Metal backend stores config for Metal API dispatch.
///
/// **CPU fallback:** setGrid/setBlock are no-ops, kernel runs once (threadIdx=0).
///
/// @code
/// static auto myKernel = gpu::NautilusKernelFunction{"vecAdd",
///     [](val<float*> a, val<float*> b, val<float*> c) {
///         auto tid = gpu::threadIdx_x() + gpu::blockIdx_x() * gpu::blockDim_x();
///         c[tid] = a[tid] + b[tid];
///     }};
///
/// void host(val<float*> a, val<float*> b, val<float*> c, val<uint32_t> n) {
///     auto blocks = (n + val<uint32_t>(255)) / val<uint32_t>(256);
///     gpu::launch(myKernel, gpu::GridDim{blocks}, gpu::BlockDim{256}, a, b, c);
/// }
/// @endcode
template <typename F, typename... Args>
void launch(NautilusKernelFunction<F>& kernel, GridDim grid, BlockDim block, Args&&... args) {
	setGrid(grid.x, grid.y, grid.z);
	setBlock(block.x, block.y, block.z);
	kernel(std::forward<Args>(args)...);
}

} // namespace nautilus::gpu
