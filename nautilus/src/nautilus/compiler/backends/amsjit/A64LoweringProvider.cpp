...
void* AsmJitLoweringProvider::lower(std::shared_ptr<ir::IRGraph> ir, ::asmjit::JitRuntime& runtime) {
	CodeHolder code;
...
// ── Top-level function setup ──────────────────────────────────────────────────

void AsmJitLoweringProvider::LoweringContext::process() {
	const auto& funcOp = ir->getRootOperation();
	const auto& funcBlock = funcOp.getFunctionBasicBlock();

	// Build the function signature from the entry block's arguments, which are
	// the actual function parameters.  (funcOp.getInputArgs() may be empty.)
	...
	// Pre-create labels for all blocks so forward jumps can be resolved.
	for (auto& block : funcOp.getBasicBlocks()) {
		getOrCreateLabel(block->getIdentifier());
	}

	// Map the entry block's arguments to the function's incoming parameters.
	// For narrow integer types (i8, i16) we sign-extend into a 64-bit scratch
	// register so that all downstream 64-bit operations see the correct value.
	for (size_t i = 0; i < entryArgs.size(); i++) {
		...
		// Sign/zero-extend narrow integer arguments into the full 64-bit register
		// so that subsequent 64-bit arithmetic operates on the correct value.
		if (std::holds_alternative<Gp>(reg)) {
			auto gReg = toGp(reg);
			if (stamp == Type::i8) {
	...
	// Emit code starting from the entry block.
...
void AsmJitLoweringProvider::LoweringContext::processCompare(ir::CompareOperation* op, RegisterFrame& frame) {
	...
	if (leftIsFloat) {
		...
		// ucomiss/ucomisd sets CF/ZF like unsigned comparison.
...
void* AsmJitLoweringProvider::lower(std::shared_ptr<ir::IRGraph> /*ir*/, ::asmjit::JitRuntime& /*runtime*/) {
	throw NotImplementedException("AsmJit backend is only implemented for x86-64");
}
