...
Block& ExecutionTrace::processControlFlowMerge(operation_identifier oi) {
	...
	// Save the blockId for later use (avoid holding reference across vector modifications)
	uint16_t referenceBlockId = blocks[oi.blockIndex].blockId;

	// Get the size before we start copying
	size_t operationsToMove = blocks[oi.blockIndex].operations.size() - oi.operationIndex;

	// Set merge block type
	getBlock(mergedBlockId).type = Block::Type::ControlFlowMerge;

	// 1. move operation indices to new block
	// move everything from the reference block between opId and end to merge block
	for (size_t i = 0; i < operationsToMove; i++) {
		uint32_t opIndex = oi.operationIndex + i;
		auto globalOpIndex = blocks[oi.blockIndex].operations[opIndex];
		...
		auto operationReference = getBlock(mergedBlockId).addOperation(globalOpIndex);
		// update in global and local tag map

		if (opType == RETURN) {
			for (auto& returnRef : returnRefs) {
				if (returnRef.blockIndex == referenceBlockId && returnRef.operationIndex == opIndex) {
	...
	blocks[oi.blockIndex].operations.erase(blocks[oi.blockIndex].operations.begin() + oi.operationIndex,
	                                       blocks[oi.blockIndex].operations.end());
	...
	blocks[oi.blockIndex].addOperation(jmpOp1);
	...
	blocks[currentBlockIndex].addOperation(jmpOp2);
