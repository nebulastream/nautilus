...
struct Trace {
	...
	bool checkTag(Snapshot& snapshot) {
		...
		if (auto* ref = globalTagMap.find(snapshot)) {
	...
	void processControlFlowMerge(TracerOperationIdentifier oi) {
		...
		for (auto operation : operations) {
			auto tag = globalTagMap.find(operation.view->tag);
			if (tag == nullptr) {
				continue;
			}
	...
	size_t capacity;
	size_t maxReturns;
	SharedHashMap<Snapshot, TracerOperationIdentifier> globalTagMap =
	    SharedHashMap<Snapshot, TracerOperationIdentifier>::create(1024 * 1024);
