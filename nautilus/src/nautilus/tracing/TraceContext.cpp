...
TypedValueRef TraceContext::traceConstValue(Type type, const ConstantLiteral& constValue) {
	...
	if (auto ref = state->executionTrace.globalTagMap.find(tag)) {
		...
		state->executionTrace.globalTagMap[tag] = oi;
		return result;
	}
}
...
bool TraceContext::traceCmp(const TypedValueRef& targetRef, const double probability) {
	...
	if (forkExecution(state->executionTrace) == ForkRole::PARENT) {
		...
		TRACER_LOG("TRUE BRANCH using {}", state->executionTrace.getOperation(oi).arguments<CmpInput>().trueBlock);
		state->executionTrace.getOperation(oi).arguments<CmpInput>().falseBlock =
		    std::numeric_limits<std::uint16_t>::max();
		return true;
	} else {
		auto oi = *state->executionTrace.globalTagMap.find(tag);
		TRACER_LOG("Updated CMP position in false block: Block: {} Operation: {}. Pointing to block: {}",
		           oi.blockIndex.offset, oi.operationOffset.offset,
		           state->executionTrace.getOperation(oi).arguments<CmpInput>().falseBlock);
		state->executionTrace.getOperation(oi).arguments<CmpInput>().falseBlock =
		    state->executionTrace.createNewBlock().getBlockId();

		TRACER_LOG("Updated CMP false block points to: {}. True branch is: {}",
		           state->executionTrace.getOperation(oi).arguments<CmpInput>().falseBlock,
		           state->executionTrace.getOperation(oi).arguments<CmpInput>().trueBlock
		);
		return false;
...
std::unique_ptr<ExecutionTrace> TraceContext::trace(std::function<void()>& traceFunction,
	...
	if (pid == 0) {
		...
		int status;
		waitpid(pid, &status, 0);
		assert(status == 0 && "Tracing Failed");
	...
	fmt::println(stderr, "{} Forks, Max Depth: {}", tracerTrace.view->forkCount, tracerTrace.view->maxForkDepth);
