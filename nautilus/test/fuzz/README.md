...
| `SeedGen.cpp`      | `nautilus-fuzz-seedgen`: regenerates `corpus/` (see "Seed corpus and dictionary"). |
| `dictionary`       | libFuzzer `-dict` file of boundary-constant byte patterns (see below). |
| `corpus/`          | Committed seed inputs exercising boundary constants and known bug shapes (see below). |
...
./nautilus/test/fuzz/nautilus-fuzz -dict=../nautilus/test/fuzz/dictionary \
    -max_total_time=120 ../nautilus/test/fuzz/corpus        # or -runs=100000
```
...
Passing `-dict=.../dictionary` and the `corpus/` directory is optional (the
fuzzer runs without them) but strongly recommended — see "Seed corpus and
dictionary" below for why libFuzzer's own mutation can almost never reach
these bytes on its own. libFuzzer treats the trailing path argument as a
corpus directory to seed from *and* write newly-discovered inputs into, so
point it at a scratch copy of `corpus/` for a long unattended run if you don't
want the committed seeds to pick up libFuzzer's own mutations:
`cp -r ../nautilus/test/fuzz/corpus /tmp/fuzz-corpus && ./nautilus-fuzz -dict=... /tmp/fuzz-corpus`.
...
`ByteReader.hpp` decodes every constant, argument, and buffer value straight
out of raw fuzzer bytes (`ByteReader::consume<T>()`), so semantically
interesting values — `INT_MIN`, `INT_MAX`, `-1`, powers of two, `NaN`,
`+-inf`, `+-0.0`, subnormals, epsilons — are only ever reached by chance: for
a `T` as wide as `i64`/`f64`, drawing the exact right 8 bytes at random is
effectively never. The "Known findings" section documents a concrete
consequence: the signed-zero negation bug was unreachable by the fuzzer until
an unrelated change made `0.0` common as a *side effect* of buffer-exhaustion
zero-padding, not because the fuzzer was ever deliberately generating it.

Two standard levers close this gap:

* **`dictionary`** (libFuzzer `-dict` format): the little-endian byte
  encoding of each boundary value, one entry per `(TypeId, value)` pair,
  covering every width in `Types.hpp::ALL_TYPES`. libFuzzer's mutator splices
  dictionary tokens into inputs at essentially any offset, so a token landing
  inside a `Kind::Const` leaf's `reader.consume<T>()` window reproduces one
  of these constants directly — see the file's own header comment for the
  derivation (plain `std::numeric_limits<T>` memcpy'd little-endian; stable,
  no regeneration needed).
* **`corpus/`**: a committed seed corpus whose inputs decode — via the exact
  `ByteReader` layout `Ast.hpp`'s generator consumes — to small programs that
  already exercise those boundary constants directly (e.g. a bare
  `Kind::Const` leaf holding `INT64_MIN`) and a handful of the specific bug
  shapes from "Known findings" below: `Neg(MIN)` (integer two's-complement
  negation overflow, the integer twin of the float signed-zero bug),
  `Div(MIN, -1)` (exercises `safeDivisor`'s steer-away-from-`-1` branch),
  `Add(MAX, 1)` and `Eq(Add(MAX, 1), 0)` (narrow-int wrap-then-compare),
  `Div(0.0, 0.0)`/`Div(+-1.0, 0.0)` (IEEE-754 division-by-zero results), and
  narrowing/`NaN`/`inf` casts at `hiLimitExclusive`/`loLimitInclusive`'s clamp
  boundary (`Types.hpp`).

  Hand-encoding a `ByteReader`-decodable byte stream is error-prone to do by
  hand and silently drifts out of sync if `Ast.hpp`'s generator ever changes,
  so `corpus/` is generated by `SeedGen.cpp` (built as
  `nautilus-fuzz-seedgen`, same opt-in as `nautilus-fuzz-replay` — no
  libFuzzer dependency): an encode-only mirror of `ByteReader`/`generateNode`
  that replays the generator's own leaf/internal-node/kind-table decisions in
  reverse, looking up every `Kind`/`TypeId` table index by *value* against
  `Ast.hpp`'s real `INT_KINDS`/`FLOAT_KINDS`/`BOOL_KINDS`/`Types.hpp`'s
  `ALL_TYPES`/`ROOT_TYPES` (never a hand-copied index), so a reordering or
  resizing of those tables is picked up automatically the next time the
  corpus is regenerated:

  ```bash
  cmake -DENABLE_FUZZER_REPLAY=ON ..
  cmake --build . --target nautilus-fuzz-seedgen
  ./nautilus/test/fuzz/nautilus-fuzz-seedgen ../nautilus/test/fuzz/corpus
  ```

  Regenerate whenever `Ast.hpp`'s `Kind`/generation logic changes in a way
  that could shift byte offsets (a new `Kind`, a reordered table, a changed
  `sel` bit layout in `generateNode`); `SeedGen.cpp`'s own header comment has
  the full reasoning. Every committed seed is replayable and self-checking
  like any other saved input — `nautilus-fuzz-replay
  ../nautilus/test/fuzz/corpus/seed-*.bin` should always exit 0 (mismatches
  would mean either a real regression or that the corpus is stale relative to
  the generator and needs regenerating).

`ReplayMain.cpp`'s libFuzzer-free built-in smoke corpus (`nautilus-fuzz-replay`,
no args) has no `-dict`/seed-corpus equivalent to plug into, since it draws
uniformly random bytes directly rather than going through libFuzzer's
mutation engine. `randomBuffer()` instead deterministically splices a handful
of the same boundary-constant byte patterns into a fraction of its generated
buffers (`boundaryPatterns()`/the injection block in `ReplayMain.cpp`),
reproducing the same "make edge-case constants reachable" effect without
touching this driver's reproducibility guarantee (same fixed xorshift seed,
so the same `NAUTILUS_FUZZ_ITERATIONS` value always produces the exact same
sequence of inputs) or its byte budget (constants are spliced in-place,
never inserted, so buffer lengths are unaffected).
