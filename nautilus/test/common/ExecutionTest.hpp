...
// modes (exceptionBasedTracing, lazyTracing, replayTracing) for every backend.
// The interpreter does not trace and is therefore skipped.
template <typename Body>
inline void forEachBackendWithTraceMode(Body&& body, const OptionsTweak& tweak = {}, bool include_asmjit = true) {
#ifdef ENABLE_TRACING
	static const std::vector<std::string> traceModes = {"exceptionBasedTracing", "lazyTracing", "replayTracing"};
	for (const auto& backend : availableBackends(include_asmjit)) {
