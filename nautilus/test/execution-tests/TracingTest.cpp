...
bool checkTestFile(const std::string& actual, const std::string& tracer, const std::string& category,
                   const std::string& group, const std::string& name) {
	auto tracerDirName = tracer == "fork" ? "fork-tracer" : (tracer == "symbolic" ? "symbolic-tracer" : tracer);
	auto groupDir = std::string(TEST_DATA_FOLDER) + tracerDirName + "/" + category + "/" + group + "/";
...
void runTraceTests(const std::string& tracer, const std::string& category,
	...
	for (auto& test : tests) {
		...
		DYNAMIC_SECTION(name) {
			auto executionTrace = tracing::trace(func, options);
			DYNAMIC_SECTION("tracing") {
				REQUIRE(checkTestFile(executionTrace.get()->toString(), tracer, category, "tracing", name));
			}
			auto ssaCreationPhase = tracing::SSACreationPhase();
			auto afterSSA = ssaCreationPhase.apply(std::move(executionTrace));
			DYNAMIC_SECTION("after_ssa") {
				REQUIRE(checkTestFile(afterSSA.get()->toString(), tracer, category, "after_ssa", name));
			}
			DYNAMIC_SECTION("ir") {
				auto irGenerationPhase = tracing::TraceToIRConversionPhase();
				[[maybe_unused]] auto ir = irGenerationPhase.apply(std::move(afterSSA));
				REQUIRE(checkTestFile(ir.get()->toString(), tracer, category, "ir", name));
			}
		}
	}
}

void runExpressionTests(const std::string& tracer) {
	SECTION("Expression Trace Test") {
		...
		runTraceTests(tracer, "expression-tests", tests);
	}
}

void runControlFlowTests(const std::string& tracer) {
	SECTION("Control-flow Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "control-flow-tests", tests);
	}
}

void runLoopTests(const std::string& tracer) {
	SECTION("Loop Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "loop-tests", tests);
	}
}

void runRuntimeCallTests(const std::string& tracer) {
	SECTION("Runtime Call Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "runtime-call-tests", tests);
	}
}

void runEnumTests(const std::string& tracer) {
	SECTION("Enum Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "enum-tests", tests);
	}
}

void runBoolTests(const std::string& tracer) {
	SECTION("Bool Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "bool-tests", tests);
	}
}

void runStaticTests(const std::string& tracer) {
	SECTION("Static Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "static-loop-tests", tests);
	}
}

void runPointerTests(const std::string& tracer) {
	SECTION("Pointer Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "pointer-tests", tests);
	}
}

void runCastTests(const std::string& tracer) {
	SECTION("Cast Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		...
		runTraceTests(tracer, "cast-tests", tests);
	}
}
...
void runRegressionsTests(const std::string& tracer) {
	SECTION("Regressions") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
		    {"store_mising_downcast-gh_#90", details::createFunctionWrapper(store_missing_downcast)}};
		runTraceTests(tracer, "regressions", tests);
	}
}

void runSelectTests(const std::string& tracer) {
	SECTION("Select Trace Test") {
		auto tests = std::vector<std::tuple<std::string, std::function<void()>>> {
...
void runTestSuite(const std::string& tracer) {
	runExpressionTests(tracer);
	runControlFlowTests(tracer);
	runLoopTests(tracer);
	runRuntimeCallTests(tracer);
	runEnumTests(tracer);
	runBoolTests(tracer);
	runStaticTests(tracer);
	runPointerTests(tracer);
	runCastTests(tracer);
	runRegressionsTests(tracer);
	runSelectTests(tracer);
}

TEST_CASE("Fork Tracer") {
	runTestSuite("fork");
}

TEST_CASE("Symbolic Tracer") {
	runTestSuite("symbolic");
}
} // namespace nautilus::engine
