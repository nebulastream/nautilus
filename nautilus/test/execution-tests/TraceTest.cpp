...
// Stubs for TypedValueRefHolder dependencies (not needed for Trace tests)
namespace nautilus::tracing {
void allocateValRef(uint16_t) {
}
void freeValRef(uint16_t) {
}
} // namespace nautilus::tracing

// Stub for Type printing (used by Catch2 for assertion output)
namespace nautilus {
...
TEST_CASE("Trace Basic Operations") {
	...
	SECTION("Append jump") {
		...
		REQUIRE(oi1.operationOffset.offset == sizeof(Trace::TraceHeader) + sizeof(Trace::BlockHeader) + sizeof(Trace::OperationHeader) + sizeof(JmpInput));
	}
}
