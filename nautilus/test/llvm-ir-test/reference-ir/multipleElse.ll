...
define signext range(i32 1, 5) i32 @execute(i32 %0) local_unnamed_addr #0 {
  %switch.selectcmp = icmp eq i32 %0, 5
  %switch.select = select i1 %switch.selectcmp, i32 3, i32 4
  %switch.selectcmp1 = icmp eq i32 %0, 10
  %switch.select2 = select i1 %switch.selectcmp1, i32 1, i32 %switch.select
  ret i32 %switch.select2
}

; Function Attrs: mustprogress nofree norecurse nosync nounwind willreturn memory(none)
define signext range(i32 1, 5) i32 @_mlir_ciface_execute(i32 %0) local_unnamed_addr #1 {
