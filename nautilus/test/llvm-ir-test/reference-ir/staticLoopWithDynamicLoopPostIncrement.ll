...
iter.check:                                       ; preds = %.lr.ph.preheader
  ...
  br i1 %min.iters.check, label %.lr.ph.preheader56, label %vector.main.loop.iter.check

vector.main.loop.iter.check:                      ; preds = %iter.check
  %min.iters.check42 = icmp ult i32 %4, 32
...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %reduced.phi = phi <8 x i32> [ <i32 180, i32 0, i32 0, i32 0, i32 0, i32 0, i32 0, i32 0>, %vector.ph ], [ %bin.rdx47, %vector.body ]
  %bin.rdx47 = add <8 x i32> %reduced.phi, splat (i32 132)
  %index.next = add nuw i32 %index, 32
  %5 = icmp eq i32 %index.next, %n.vec
  br i1 %5, label %middle.block, label %vector.body, !llvm.loop !1

middle.block:                                     ; preds = %vector.body
  %6 = tail call i32 @llvm.vector.reduce.add.v8i32(<8 x i32> %bin.rdx47)
...
vec.epilog.iter.check:                            ; preds = %middle.block
  ...
  br i1 %min.epilog.iters.check, label %.lr.ph.preheader56, label %vec.epilog.ph
...
vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %index50 = phi i32 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next52, %vec.epilog.vector.body ]
  %vec.phi51 = phi <4 x i32> [ %8, %vec.epilog.ph ], [ %9, %vec.epilog.vector.body ]
  %9 = add <4 x i32> %vec.phi51, splat (i32 33)
  %index.next52 = add nuw i32 %index50, 4
  %10 = icmp eq i32 %index.next52, %n.vec49
...
vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  ...
  %cmp.n53 = icmp eq i32 %4, %n.vec49
  br i1 %cmp.n53, label %._crit_edge, label %.lr.ph.preheader56

.lr.ph.preheader56:                               ; preds = %vec.epilog.iter.check, %vec.epilog.middle.block, %iter.check
  %.ph = phi i32 [ 6, %iter.check ], [ %ind.end, %vec.epilog.iter.check ], [ %7, %vec.epilog.middle.block ]
  %.ph57 = phi i32 [ 180, %iter.check ], [ %6, %vec.epilog.iter.check ], [ %11, %vec.epilog.middle.block ]
  br label %.lr.ph

.lr.ph:                                           ; preds = %.lr.ph.preheader56, %.lr.ph
  %12 = phi i32 [ %15, %.lr.ph ], [ %.ph, %.lr.ph.preheader56 ]
  %13 = phi i32 [ %14, %.lr.ph ], [ %.ph57, %.lr.ph.preheader56 ]
  %14 = add i32 %13, 33
