...
void addIndirectCall(ExecutionTrace& trace, Block& block, TypedValueRef fnPtr, std::vector<TypedValueRef> arguments,
	...
	auto* call = trace.getArena().create<IndirectFunctionCall>(IndirectFunctionCall {
	    .fnPtr = fnPtr, .arguments = std::move(arguments), .fnAttrs = {}, .exceptionCapture = std::nullopt});
	auto snapshot = Snapshot();
