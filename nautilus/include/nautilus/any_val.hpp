...
public:
	any_val() = default;
	any_val(const any_val&) = default;
	any_val(any_val&&) = default;
	any_val& operator=(const any_val&) = default;
	any_val& operator=(any_val&&) = default;
	virtual ~any_val() = default;
...
/// Checks whether an any_val pointer holds a specific val<T> type.
///
...
[[nodiscard]] bool isa(const any_val* v) {
...
template <typename ValT>
[[nodiscard]] const ValT* cast(const any_val* v) {
	assert(v && "cast called on null pointer");
	assert(isa<ValT>(v) && "cast<> type mismatch");
...
/// ```
template <typename ValT>
[[nodiscard]] ValT* dyn_cast(any_val* v) {
	if (v && isa<ValT>(v)) {
		return static_cast<ValT*>(v);
	}
	return nullptr;
...
template <typename ValT>
[[nodiscard]] const ValT* dyn_cast(const any_val* v) {
	if (v && isa<ValT>(v)) {
		return static_cast<const ValT*>(v);
	}
	return nullptr;
