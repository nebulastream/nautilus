...
serializeFunctionWithDependencySymbols(Function& inlineFunction) {
	...
	if (inlineFunction.hasPersonalityFn()) {
		...
		if (auto* srcPersonality = dyn_cast<Function>(srcPersonalityConst)) {
			auto* clonedPersonality =
			    Function::Create(srcPersonality->getFunctionType(), srcPersonality->getLinkage(),
			                     srcPersonality->getAddressSpace(), srcPersonality->getName(), &wrapperModule);
