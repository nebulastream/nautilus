...
public:
	...
	/// Called after each invocation of a region's body. Returns true to request
	/// another invocation (another local exploration pass over an unresolved
	/// internal branch) and prepares state for it; returns false when the
	/// region is fully resolved and the caller should stop invoking it.
	/// Implementations that don't support region-local exploration (the root
	/// contexts, and a region body that is only being transparently followed)
	/// always return false, preserving today's single-invocation behavior.
	virtual bool traceRegionContinue() = 0;
