...
| `engine.resolveFunctionNames` | `false` (`true` if `debug`, `perf` or `perf.sample` is set) | Looks up the symbol name of every called runtime function with `dladdr` after tracing, cached process-wide. `dladdr` scans the symbol table of the containing object and can cost about 1 ms per callee in large binaries, so by default names are only resolved when the compiled code will be inspected. When disabled, callees are named by their address; the generated code is unaffected. Setting it explicitly overrides the default. |
