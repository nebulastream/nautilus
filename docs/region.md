...
- **A value built in a single branch arm cannot be carried out of a region as a live value under `lazyTracing`.** Constructing a `val<T>` inside one arm of an `if` and keeping it alive past the region (in a container or `std::optional` declared outside it) has no merge point — what escapes depends on which arm ran, much like returning a value from the region lambda. `exceptionBasedTracing` traces one arm per engagement and handles it; `lazyTracing` runs both arms against the same enclosing object and reports an error rather than producing a wrong trace. Building the value unconditionally, or escaping by *assignment* to a captured `val<T>` (the idiom used throughout this document), works under both tracers.
