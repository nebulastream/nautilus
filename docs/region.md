...
The MLIR backend reads it: when `mlir.debug.enable` is on, every op traced inside a region is lowered as if it had been inlined out of a synthetic, region-named function — a real DWARF `DW_TAG_inlined_subroutine`, not a plain lexical block. A lexical block was the first design tried here, but a lexical block is never a stack-frame boundary and GDB prints exactly one source line per frame, so no amount of nesting plain scopes ever makes a region show up as its own line in `bt`. Modeling `region("accumulate", ...)` as "as if inlined" reuses the exact mechanism GDB already has for showing an `-O2`-inlined C++ function as its own backtrace entry: a synthetic `LLVM::DISubprogramAttr` stands in for the region (one instance per region, shared by every op traced inside it, however many Nautilus basic blocks — a loop's header and body, say — that ends up being), and each op's location is wrapped in an MLIR `CallSiteLoc` chain (one level per region nesting) pointing back out to the enclosing function's own subprogram. Verified against a real compiled binary: `bt` shows `#0 accumulate () at ... / #1 execute () at ...` consistently at every instruction address across the region's extent, and the frame disappears again exactly at the region's boundary; `readelf --debug-dump=info` shows the corresponding `DW_TAG_inlined_subroutine` with a `DW_AT_abstract_origin` pointing at the region's own `DW_TAG_subprogram`.

The region's synthetic subprogram always stays filed under the function's own DWARF file, never the region's real source file: a `DILocation` has no file of its own, so giving it a different file would silently reinterpret every op's dump-relative line as a line in that unrelated file. What is per-region rather than per-op is the line of each *enclosing* frame: the region's own synthetic subprogram is shared across every op traced inside it, and each ancestor level is located at the line where the region below it opens (its `CallSiteLoc`'s `inlinedAt`) — correct DWARF, since an inlined frame shows its call site. Individual ops inside the region keep their own, distinct line: `wrapOpForRegionInlining` wraps the op's own untouched location as the `CallSiteLoc`'s callee, so `perf annotate` (or a debugger single-stepping the inlined frame) still attributes each instruction to the exact source line it came from, not to one collapsed synthetic location for the whole region. It is also deliberately never an `LLVM::DILexicalBlockFileAttr`, even though that construct looks like the natural fit for "this scope's file differs from its parent's" — LLVM's DWARF backend treats `DILexicalBlockFile` as fully transparent when building the lexical-scope tree, so it can never surface as a real, observable scope. The other backends (`cpp`, `bc`, `tbc`, `asmjit`) do not read region metadata yet; for them it remains provenance for reading, verifying and debugging the IR.

**This reaches a debugger directly; `perf` gets it a different way.** The `DW_TAG_inlined_subroutine` above lives in the JIT-linked object, which GDB and LLDB read through the JIT registration interface (`mlir.debug.register_with_debugger`). `perf` never reads that object. Under `mlir.perf.enable` the profile is built from the perf *jitdump* instead, and `perf inject --jit` synthesizes its own ELF per code range from that file alone -- so only what the jitdump can carry survives. The format has nowhere to put a scope tree: its `JIT_CODE_DEBUG_INFO` record is a flat line table (`{code_addr, nr_entry}` followed by `{addr, line, discrim, file}` entries), with no subprogram records and no inline nesting. A generated `jitted-*.so` inspected with `readelf --debug-dump=info` contains a bare `DW_TAG_compile_unit` carrying only `DW_AT_stmt_list`: no `DW_TAG_subprogram`, no `DW_TAG_inlined_subroutine`. Left at that, every region collapsed silently into its enclosing function's box in a flame graph -- silently, because the function symbols still resolved, so a profile looked correct rather than incomplete.

So the region nesting is re-expressed in the one channel the format does carry: symbol names. `PerfJitDumpPlugin` walks the DWARF inline stack per address -- the very chain described above -- and emits one `JIT_CODE_LOAD` per contiguous run of code sharing a region stack, named for its nesting (`execute::outer::hot`), splitting the enclosing function's own symbol around it so the ranges stay disjoint as the format requires. **Regions are therefore sibling symbols in a profile, not nested frames**: a flame graph shows `execute::outer::hot` beside `execute`, not stacked on it. `mlir.perf.region_symbols=false` restores plain per-function symbols. A Nautilus-to-Nautilus call inlined into a region is attributed to that region rather than named separately -- naming it would alternate `...::hot` and `...::hot::callee` ranges, and perf keys a symbol by address rather than name, so it never recombines them. `docs/profiling.md` covers the workflow; `nautilus/test/execution-tests/PerfJitDumpExecutionTest.cpp` pins the behaviour.
