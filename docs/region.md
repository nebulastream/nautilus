...
- **Under `lazyTracing`, a region's internal control flow must be the last thing in the region body.** Region-local exploration re-invokes the body once per pass, and two shapes it cannot currently express are rejected with an error naming `region()` and the trace mode rather than being traced incorrectly:
  - *Traced operations after an `if`/loop inside the region body* (including operations that follow a nested `region()`). Tracing leaves an empty, unterminated basic block behind, so this is reported at the end of tracing.
  - *A `val<T>` created inside the region and still alive when the region ends*, when the region also contains internal control flow — for example one stored into a container or `std::optional` declared outside the region. The per-pass reset of the region's liveness bookkeeping would discard a reference its eventual destruction still releases.

  Both work correctly under `engine.traceMode = "exceptionBasedTracing"`, which invokes the region body once per engagement; the cost is losing the region's branch-tracing speedup. A region whose branch or loop is the last thing in its body — the shape the O(N²) → O(N) argument above is about — is unaffected, as is escaping a value by *assignment* to a captured `val<T>`, the idiom used throughout this document.
