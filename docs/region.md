...
**This reaches a debugger, but not `perf`.** The `DW_TAG_inlined_subroutine` above lives in the JIT-linked object, which GDB and LLDB read through the JIT registration interface (`mlir.debug.register_with_debugger`). `perf` never reads that object. Under `mlir.perf.enable` the profile is built from the perf *jitdump* instead, and `perf inject --jit` synthesizes its own ELF per code range from that file alone — so only what the jitdump can carry survives. The format has nowhere to put a scope tree: its `JIT_CODE_DEBUG_INFO` record is a flat line table (`{code_addr, nr_entry}` followed by `{addr, line, discrim, file}` entries), with no subprogram records and no inline nesting. Inspecting a generated `jitted-*.so` with `readelf --debug-dump=info` shows a bare `DW_TAG_compile_unit` carrying only `DW_AT_stmt_list`: no `DW_TAG_subprogram`, no `DW_TAG_inlined_subroutine`, and no region name anywhere in the file.

The practical consequence is that **every region collapses into its enclosing function's box in a flame graph**, and `perf report --inline` lists only the traced functions. This is a silent degradation — the function symbols still resolve, so a profile looks correct rather than incomplete. What does survive is the per-op line numbers, so `perf annotate` still points at the right lines; recovering which *region* those lines belong to means cross-referencing them against the region legend at the foot of the Nautilus IR dump by hand, and that dump is written to a temp directory unless `mlir.debug.source_dir` says otherwise. Two tests tagged `[!shouldfail]` in `nautilus/test/execution-tests/PerfJitDumpExecutionTest.cpp` pin this behaviour so it cannot regress further and so a fix is noticed when it lands.
