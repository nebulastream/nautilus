...
- **A value that is still alive when the region ends cannot escape a region containing internal control flow, under `lazyTracing`.** Constructing a `val<T>` inside a region and keeping it alive past the region's end (by storing it in a container or `std::optional` declared outside the region) works for a straight-line region under both tracers, and works under `exceptionBasedTracing` even when the region contains an `if` or a loop. Under `lazyTracing` it currently fails when the region also contains internal control flow: local branch exploration re-invokes the region body once per pass and resets the region's liveness bookkeeping between passes, which discards refs that are still alive in an enclosing C++ scope. Escaping a value by *assignment* to a captured `val<T>` — the idiom used throughout this document — is unaffected and works in every case.
