...
Attributing the operations is what makes the metadata survive the pipeline: the entry and exit block a region adds are single-predecessor seams that `EmptyBlockEliminationPass` and `BlockMergingPass` collapse (see [How regions trace branches](#how-regions-trace-branches)), and a block that swallows a region's code widens to the region containing both — while the operations themselves keep naming their own region wherever they end up. Blocks that survive whole, like the arms of a branch inside a region body, keep naming it directly.

A pass that replaces one operation with another does not have to remember to copy this metadata by hand: "a replacement inherits what it replaces" is the rule `FunctionRewriter`'s creation primitives enforce uniformly. `createBefore(anchor, ...)` stamps the operation it mints with `anchor`'s own provenance -- every call site across the codebase passes the operation being replaced as the anchor, so constant folding and algebraic simplification get this for free. `createBeforeTerminator` and `replaceTerminator` have no such anchor to read from (the former inserts before a block's terminator, not before the code it is standing in for; the latter's caller builds the replacement terminator standalone) -- the former is stamped from whatever `FunctionRewriter::setCurrentProvenance` was last told, which a pass sets explicitly before minting (e.g. `StrengthReductionPass` pointing new preheader/latch code back at the multiply-add it strength-reduces), and the latter copies the terminator it replaces automatically. Either way, when the thing being inherited from is itself unattributed, the fallback is not `NO_REGION` but the region of the block the new operation lands in: a block's region is, by construction, a region every operation inside it already belongs to (or is nested inside), so defaulting to it is never wrong, only sometimes less precise than what an anchor would have said. `NO_REGION` remains the answer only where the block itself has none. Code with no traced ancestor at all -- a landing pad's destructor calls, synthesized rather than traced, into a freshly created block that was never assigned a region -- stays unattributed for exactly that reason: the point of the distinction is "no traced origin", not "nobody bothered" (issue #453).
