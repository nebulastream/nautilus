...
**Code that calls into Nautilus must keep its frame pointers.** The tracer identifies each traced instruction by walking the caller's stack frames, which requires every frame on the stack to have a frame pointer -- `-fno-omit-frame-pointer`, which compilers omit by default at `-O1` and above. Linking against the `nautilus` CMake target applies the flag automatically; compiling against the library by hand does not, and the resulting tags are derived from whatever happens to sit where the saved frame pointer should be. Nautilus checks the frame chain as it walks and reports a `TagCreationException` naming the flag rather than following a bogus pointer, but the check is best-effort: data that happens to look like a frame pointer is indistinguishable from a real one.
