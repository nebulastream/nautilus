...
| engine.traceMode=[lazyTracing,exceptionBasedTracing,stackCopyTracing,forkTracing] | lazyTracing | Selects the tracer implementation. The first two re-execute the function once per control-flow path; stackCopyTracing and forkTracing execute every path exactly once by snapshotting execution state at branches (see docs/tracing.md for the trade-offs). |
| engine.tracePruning=[true,false]            | false    | Skips exploration of branches whose condition is a trace-time constant boolean: the statically dead side is never traced and no CMP is recorded. Branch probability hints are never used for pruning. |
| engine.traceStackSize=[bytes]               | 8388608  | Size of the side stack on which stackCopyTracing runs the traced function. |
| engine.forkTraceTimeoutMs=[ms]              | 300000   | Watchdog timeout after which forkTracing kills the worker process tree and reports an error. |
