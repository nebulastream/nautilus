...
This is Linux/ELF only. On any other platform `perf` is a no-op (a warning is
printed once) rather than an error.
...
- **The dump directory**: `perf` alone (without `debug`) writes its Nautilus-IR
  "source" dump to the working directory rather than `$TMPDIR`, precisely so
  it outlives the compile -- the dump must still exist, unmoved, when `perf
  inject`/`perf annotate` run, which can be minutes or hours later.
...
builds for `debug`'s GDB path -- which trades away the reason
...
Setting `debug` and `perf` together still works: the jitdump gets a line
table, and you can attach a debugger to the same process. But `debug`'s
optimization clamp (`-O0`, to keep `$N` values readable while stepping) wins,
so the resulting profile measures debug-clamped code, not the
production-optimized code this feature otherwise gets you. A one-time warning
is printed when both are set. Pass `optimizationLevel` explicitly to override
the clamp if you need both a debugger and a representative profile at once.
