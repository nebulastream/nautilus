...
public:
	...
	template <typename T, FMT_ENABLE_IF(nautilus_fmt::internal::is_integral<T>::value)>
...
int basic_printf_context<OutputIt, Char>::parse_header(const Char*& it, const Char* end, format_specs& specs) {
	...
	if (it != end) {
		if (*it >= '0' && *it <= '9') {
			...
			++it;
			specs.width =
			    static_cast<int>(visit_format_arg(internal::printf_width_handler<char_type>(specs), get_arg()));
		}
	}
	return arg_index;
}
...
OutputIt basic_printf_context<OutputIt, Char>::format() {
	...
	while (it != end) {
		...
		// Parse type.
		// if (it == end)
		// FMT_THROW(duckdb::InvalidInputException("invalid format string"));
