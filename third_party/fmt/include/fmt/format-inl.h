...
inline nautilus_fmt::internal::null<> strerror_r(int, char*, ...) {
	return {};
}

inline nautilus_fmt::internal::null<> strerror_s(char*, std::size_t, ...) {
	return {};
}
...
FMT_ALWAYS_INLINE digits::result grisu_gen_digits(fp value, uint64_t error, int& exp, Handler& handler) {
	...
	do {
		...
		result = handler.on_digit(static_cast<char>('0' + digit), data::powers_of_10_64[exp] << -one.e, remainder,
		                          error, exp, true);
		if (result != digits::more)
			return result;
	} while (exp > 0);
