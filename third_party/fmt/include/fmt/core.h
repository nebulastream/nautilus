...
template <>
struct is_char<fmt_char8_t> : std::true_type {};
template <>
...
using nautilus_fmt::v6::to_string_view;
...
make_args_checked(const S& format_str, const remove_reference_t<Args>&... args) {
	static_assert(
	    all_true<(!std::is_base_of<view, remove_reference_t<Args>>() || !std::is_reference<Args>())...>::value,
