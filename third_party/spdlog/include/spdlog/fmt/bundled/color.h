...
public:
    ...
    FMT_CONSTEXPR auto has_foreground() const noexcept -> bool { return set_foreground_color; }
    FMT_CONSTEXPR auto has_background() const noexcept -> bool { return set_background_color; }
...
inline void vprint(std::FILE* f, const text_style& ts, string_view fmt, format_args args) {
    ...
    if (result < 0) FMT_THROW(system_error(errno, FMT_STRING("cannot write to file")));
}
...
inline auto vformat(const text_style& ts,
                    const S& format_str,
                    basic_format_args<buffer_context<type_identity_t<Char>>> args)
...
inline auto format(const text_style& ts, const S& format_str, const Args&... args)
    -> std::basic_string<Char> {
