...
#if (FMT_CPLUSPLUS >= 202002L || (FMT_CPLUSPLUS >= 201709L && FMT_GCC_VERSION >= 1002)) && \
...
FMT_CONSTEXPR inline auto is_utf8() -> bool {
    ...
    return FMT_UNICODE ||
           (sizeof(section) == 3 && uchar(section[0]) == 0xC2 && uchar(section[1]) == 0xA7);
}
}  // namespace detail
...
public:
    ...
    FMT_CONSTEXPR_CHAR_TRAITS friend auto operator==(basic_string_view lhs, basic_string_view rhs)
        -> bool {
...
template <typename T>
class iterator_buffer<T*, T, fixed_buffer_traits> final : public fixed_buffer_traits,
                                                          public buffer<T> {
private:
    T* out_;
    enum { buffer_size = 256 };
...
public:
    ...
    FMT_CONSTEXPR20 FMT_INLINE value(T& val) {
        ...
        custom.format =
            format_custom_arg<value_type, typename Context::template formatter_type<value_type>>;
    }
    value(unformattable);
...
private:
    ...
    friend FMT_CONSTEXPR auto visit_format_arg(Visitor&& vis, const basic_format_arg<Ctx>& arg)
        -> decltype(vis(0));
...
template <>
inline auto find<false, char>(const char* first, const char* last, char value, const char*& out)
    -> bool {
...
FMT_CONSTEXPR auto do_parse_arg_id(const Char* begin, const Char* end, Handler&& handler)
    -> const Char* {
    Char c = *begin;
    if (c >= '0' && c <= '9') {
        int index = 0;
...
template <typename Char, typename Handler>
FMT_CONSTEXPR FMT_INLINE auto parse_arg_id(const Char* begin, const Char* end, Handler&& handler)
    -> const Char* {
...
template <typename Char>
FMT_CONSTEXPR auto parse_dynamic_spec(const Char* begin,
                                      const Char* end,
                                      int& value,
                                      arg_ref<Char>& ref,
...
FMT_CONSTEXPR FMT_INLINE auto parse_format_specs(const Char* begin,
    ...
    for (;;) {
        switch (c) {
            ...
            case 's':
                return parse_presentation_type(pres::string, bool_set | string_set | cstring_set);
            case 'p':
...
template <typename Char, typename Handler>
FMT_CONSTEXPR auto parse_replacement_field(const Char* begin, const Char* end, Handler&& handler)
    -> const Char* {
    struct id_adapter {
        Handler& handler;
        int arg_id;
...
FMT_CONSTEXPR auto parse_format_specs(ParseContext& ctx) -> decltype(ctx.begin()) {
    ...
    using mapped_type = conditional_t<mapped_type_constant<T, context>::value != type::custom_type,
