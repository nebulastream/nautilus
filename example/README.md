...
| `./demo_perf_profile` | [src/DemoPerfProfile.cpp](src/DemoPerfProfile.cpp) | A kernel shaped for `perf`: nested `region()`s of deliberately unequal cost (`query` > `scan` / `aggregate` > `mix` / `merge`), a Nautilus-to-Nautilus call inlined into a region, and an `invoke()` into a host function. Run it under [tools/nautilus-perf.sh](../tools/nautilus-perf.sh) for a symbolized profile, a flame graph with the region nesting stacked (`flamegraph`), and the whole Nautilus-IR dump with a per-line percentage gutter (`annotate-ir`). See [docs/profiling.md](../docs/profiling.md). |
